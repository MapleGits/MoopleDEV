...
public class Commands {
    public static boolean executeGMCommand(MapleClient c, String[] sub, char heading) {
        ...
        if (sub[0].equals("ap")) {
			...
			if (sub.length == 1) {
				player.dropMessage("Syntax: !map <Map ID> <Portal ID>");
			} else {
				MapleMap target = null;
				try {
					target = cserv.getMapFactory().getMap(Integer.parseInt(sub[1]));
				} catch (Exception e) {}
				if (target == null)
					player.dropMessage("You have entered an incorrect Map ID.");
				else {
